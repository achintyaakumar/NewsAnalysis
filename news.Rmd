...
barchart(key ~ rake, data = head(subset(stats, freq > 1), 20), col = "red", #change the frequency number
...
TWITTER. 
setup_twitter_oauth("API key", "API secret", "Access token", "Private access token")
```{r}
library(twitteR)
library(devtools)
library(rtweet)
...
```{r}
library(entity)
organizations <- organization_entity(results_in_16$results_df$description)
plot(organizations)
